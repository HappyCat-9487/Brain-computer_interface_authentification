...
class TreesModel:
    ...
    def train_extra_trees_binary(self):
...
def plot_precision_recall_curve(precision_recall, trial_name, paras, treeName, n_estimator, classes, accuracy, precision, recall, f1, labels):
    #print("Precision-Recall Data:", precision_recall)  
...
if __name__ == "__main__":
    ...
    results, classes = trainer.train_extra_trees_binary()
    
    for cls, metrics in results.items():
        class_name = trainer.label_encoder.inverse_transform([cls])[0]
        print(f"{trial_name} for class '{class_name}' with {paras} parameters => Accuracy: {metrics['accuracy']}, Precision: {metrics['precision']}, Recall: {metrics['recall']}, F1: {metrics['f1']}")
